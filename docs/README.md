# Documentation

Welcome to the Tentacle documentation portal. Use the sections below to find the right set of docs for your task.

## Assets
- Image assets guide: [assets/README.md](assets/README.md)

![Tentacle vision](assets/vision/tentacle-vision.png)

## Product
- Technical specification: [product/technical-spec.md](product/technical-spec.md)
- Test specification: [product/test-specification.md](product/test-specification.md)
- Artistry and design intent: [product/artistry.md](product/artistry.md)
- Glossary: [product/glossary.md](product/glossary.md)

## Manufacturing
- Fabrication guidance: [manufacturing/fabrication.md](manufacturing/fabrication.md)
- Post-processing: [manufacturing/post-processing.md](manufacturing/post-processing.md)
- QC guidance: [manufacturing/qc-guidance.md](manufacturing/qc-guidance.md)

## Release
- Versioning and immutability rules: [release/versioning.md](release/versioning.md)
- Promotion policy: [release/promotion-policy.md](release/promotion-policy.md)
- Runbook: [release/runbook.md](release/runbook.md)
- Build and publish: [release/build-and-publish.md](release/build-and-publish.md)
- Ledger specification: [release/ledger-spec.md](release/ledger-spec.md)

## Automation
- Publish requirements: [automation/publish.requirements.md](automation/publish.requirements.md)
- Promote requirements: [automation/promote.requirements.md](automation/promote.requirements.md)
- Validate requirements: [automation/validate.requirements.md](automation/validate.requirements.md)
